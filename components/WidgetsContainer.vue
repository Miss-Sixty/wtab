...
<template>
  <div relative select-none :class="scale" :style="{ ...widgetWH }" bg-red rounded-lg>
    <template v-if="type">
      <div
        :class="[iconScale, props.type === 'add' ? 'i-solar-add-circle-bold' : '']" absolute left-0 top-0 scale-100
        cursor-pointer rounded-full transition text-2xl class="-translate-x-1/4 -translate-y-1/4 " @click="handleClick"
      />
    </template>
