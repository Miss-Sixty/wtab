...
<template>
  <Popover class="relative inline-block">
    <PopoverButton icon>
      <div i-heroicons-swatch-20-solid text-lg />
    </PopoverButton>
      ...
      <PopoverPanel class="absolute right-0 z-100 mt-2 w-54 border-1 border-base rounded-xl bg-muted px-4 py-5 shadow-lg">
        <div class="flex flex-col space-y-5">
          <div class="grid grid-cols-5 gap-3">
            <button
              v-for="[key, theme] in primaryThemes" :key="key" :style="{ background: theme['--una-primary-hex'] }"
              class="h-6.5 w-6.5 rounded-full transition-all"
              :class="[currentPrimaryThemeName === key ? 'ring-2' : 'scale-93']" ring="primary offset-4 offset-base"
          ...
          <div class="grid grid-cols-5 gap-3">
            <button
              v-for="[key, theme] in grayThemes" :key="key" :style="{ background: theme['--una-gray-hex'] }"
              :class="currentGrayThemeName === key ? 'ring-2' : 'scale-93'"
              class="h-6.5 w-6.5 rounded-full transition-all" ring="gray offset-4 offset-base"
          ...
          <div class="flex space-x-3">
            <button
              btn="~ solid block" class="transition" label="Shuffle"
              leading="i-heroicons-adjustments-horizontal-solid" :una="{
                btnLeading: value ? 'rotate-180 transform' : 'rotate-0',
              }" @click="shuffleTheme"
            />

            <button btn="~ solid-gray" size="xs" icon label="i-heroicons-arrow-uturn-left" @click="reset" />
          </div>
        </div>
      </PopoverPanel>
    </transition>
